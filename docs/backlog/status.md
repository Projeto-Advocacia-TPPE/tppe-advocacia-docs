...
| APIs Externas | Parcial | Há sincronização individual e em lote com a API pública DataJud, seleção do tribunal por alias salvo no processo, comando agendável para execução periódica, retentativa para falhas temporárias, log persistido e notificação por e-mail de falhas externas; ainda falta política mais completa de retentativas assíncronas |
...
| US-21 | Parcial | Tabela `external_api_logs`, `GET /api/v1/external-api-logs`, notificação por e-mail aos admins em falhas externas com throttling e retentativa automática para falhas temporárias do DataJud | Wrapper genérico para todas as integrações, payload sanitizado e política mais completa de retentativas assíncronas |
...
1. Configurar cron/scheduler de deploy para executar o comando agendável de sincronização DataJud.
